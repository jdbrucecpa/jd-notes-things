...
- [ ] Fix "Join Meeting" button to open the meeting link (Zoom/Teams/Meet)
- [ ] Fix "Record Meeting" button to start recording for that specific meeting
- [ ] Add distinct, clickable meeting link in the card UI
- [ ] Include the date in addition to time on meeting cards
- [ ] Ensure meeting card displays correct info when pulled from Google Calendar
...
- [ ] Create new `RecordingWidget` BrowserWindow (always-on-top, frameless)
- [ ] Small UI showing: meeting title, recording status indicator, start/stop button
- [ ] Draggable window (can be positioned anywhere)
- [ ] Visual feedback: recording indicator (red dot/pulse), elapsed time
- [ ] Settings option to enable/disable widget popup behavior
- [ ] Widget syncs state with main window recording controls
...
- [ ] Add setting: "Auto-start recording when meeting begins" (default: off)
- [ ] Add setting: "Show popup when meeting detected" (for when auto-start is off)
- [ ] Meeting detection polling (check every 30 seconds if calendar meeting is due)
- [ ] When calendar time hits:
...
- [ ] Handle overlapping meetings: show dialog asking which meeting to record
...
- [ ] Add filter dropdown: Company (extracted from routing/organization)
- [ ] Add filter dropdown: Contact (from meeting participants)
- [ ] Add filter dropdown: Platform (Zoom, Teams, Google Meet, etc.)
- [ ] Add filter toggle: Synced to Obsidian / Not synced
...
- [ ] "Save View" button to save current filter state with a name
- [ ] Views dropdown to select saved views
- [ ] Views stored in `appData/views.json`
- [ ] Predefined views: "All Meetings", "Not Synced to Obsidian"
- [ ] Edit/delete saved views
- [ ] View indicator showing current active view
...
- [ ] Create Stream Deck plugin package structure
- [ ] Implement WebSocket server in main app for Stream Deck communication
- [ ] Start Recording action (icon changes when recording)
- [ ] Stop Recording action
- [ ] Visual feedback: icon states for idle/recording/error
- [ ] Plugin configuration via Stream Deck software
...
- [ ] Add settings section for Stream Deck integration
- [ ] Toggle to enable/disable Stream Deck server
