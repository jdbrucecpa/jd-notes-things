...
| "Not synced to Obsidian" view               | UI       | ✅ Done     |
| Stale link detection/refresh                | Utility  | ✅ Done     |
...
### RS-2: Stale Link Detection & Refresh ✅ Done

**Depends on:** RD-3 (Obsidian link management research)

- [x] **RS-2.1** Add Obsidian path field to meeting data model (already exists as `obsidianLink`)
- [x] **RS-2.2** Add unique meeting ID to note frontmatter
- [x] **RS-2.3** Implement vault scan function to find notes by meeting ID
- [x] **RS-2.4** Implement link validation (check if stored path still exists)
- [x] **RS-2.5** Create "Refresh Links" function
  - Scan vault for all meeting notes
  - Match to app records by meeting ID
  - Update stored paths
- [x] **RS-2.6** Add "Refresh Links" button to Settings > General > Data Maintenance
- [x] **RS-2.7** Show results: updated, not found, unchanged
- [ ] **RS-2.8** For "not found" notes, offer file picker to manually locate (deferred - manual relocation not critical)
- [ ] **RS-2.9** Test with moved files
- [ ] **RS-2.10** Test with deleted files
- [ ] ~~**RS-2.11** Test manual relocation~~ (deferred with RS-2.8)

**Files modified:**

- `src/main.js` - Added `meeting_id` to frontmatter in `generateSummaryMarkdown()` and `generateTranscriptMarkdown()`, added `obsidian:refreshLinks` IPC handler
- `src/main/storage/VaultStructure.js` - Added `findFileByMeetingId()`, `scanAllMeetingNotes()`, `refreshObsidianLinks()`, `_extractMeetingIdFromFrontmatter()`, `_scanDirectoryForMarkdown()` methods
- `src/preload.js` - Added `obsidianRefreshLinks` API
- `src/index.html` - Added "Data Maintenance" section to Settings > General with "Refresh Links" button
- `src/renderer/settings.js` - Added click handler for refresh links button with loading state and status display

**Implementation:**

- Meeting notes now include `meeting_id` in YAML frontmatter for identification
- "Refresh Links" button in Settings > General > Data Maintenance scans entire vault
- Vault scan recursively finds all `.md` files, extracts `meeting_id` from frontmatter
- Compares stored `obsidianLink` paths with actual vault paths
- Updates meetings whose notes have been moved (stale links)
- Reports: number updated, number missing (deleted from vault)
- Toast notification shows results, status text persists in UI

**Bug fixes (Session 3):**

- Fixed `exportToObsidian()` checking wrong field (`result.data.vaultPath` → `result.obsidianLink`)
- Fixed metadata tab not showing vault path (now checks both `vaultPath` and `obsidianLink`)
- Fixed UI not updating after export (now updates header, metadata tab, sync status indicator, and calls update callback)

### RS-3: Review Section Organization ❌ Descoped

**Status:** Descoped during v1.1 development

**Reason:** After IM-3 (Import Review Queue) was descoped, this feature would only have one tab ("Not Synced"). Since RS-1 implements "Not Synced" as an inline filter toggle, creating a separate Review section with only one tab adds unnecessary complexity. The inline filter approach is simpler and more discoverable.
...
Instead of a separate Review section, the "Not Synced" filter toggle (RS-1) provides the same functionality:

- Toggle button in header shows/filters unsynced meetings
- Individual "Sync" buttons on each meeting card
- "Sync All" bulk action when filter is active
