 ...
 * Runs inside Electron so better-sqlite3's native module loads correctly.
 *
 * Usage: npx electron tests/test-database.js
 *   or:  npm run test:db
 *
...
// Headless — no GPU, no windows
app.disableHardwareAcceleration();

// Create isolated temp directory and redirect userData BEFORE
// requiring databaseService (its constructor calls app.getPath('userData'))
...
// Now safe to require — better-sqlite3 is compiled for Electron's V8
...
function runTests() {
  ...
  try {
    // Verify saveMeeting is atomic: a meeting with invalid child data
    // should not leave a partial meeting row behind
    const meetingBefore = databaseService.db
    ...
    databaseService.saveMeeting(badMeeting);
    const saved = databaseService.getMeeting('txn-test-001');
    assert(saved !== null, 'Transaction test meeting saved');
    assertEq(saved.participants.length, 1, 'Participant saved in transaction');
    assertEq(saved.transcript.length, 1, 'Transcript saved in transaction');

    // Clean up
    databaseService.deleteMeeting('txn-test-001');

    const meetingAfter = databaseService.db
      .prepare('SELECT COUNT(*) as count FROM meetings')
      .get().count;
    assertEq(meetingAfter, meetingBefore, 'No orphan rows after transaction test');
  } catch (err) {
    failed++;
    console.error('  ✗ FAIL: Transaction test threw:', err.message);
  }
...
// Run tests once Electron is ready (needed for app.getPath, etc.)
app.whenReady().then(() => {
  const failures = runTests();
  app.exit(failures > 0 ? 1 : 0);
});

// Prevent Electron from quitting when there are no windows (we're headless)
